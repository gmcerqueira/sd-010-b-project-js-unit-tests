...
const createMenu = (e) => {
  let menu = {
    fetchMenu: () => {
      menu.consumption = [];
      if (e === undefined) return { food: {}, drink: {} };
      return e;
    },
    order: (item) => {
    ...
    orderFromMenu: (request) => {
      if (request === 'request') {
        resturant.consumption = menu.consumption;
      }
    },
    pay: () => {
      let total = 0;
      resturant.consumption.forEach((index) => {
        Object.values(e).forEach((index2) => {
          if (index2[`${index}`]) total += index2[`${index}`];
        });
      });
