...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    let objetoRetornado = createMenu('teste'); // Retorno: { fetchMenu: () => {}, ... }
    assert.deepStrictEqual(typeof objetoRetornado, 'object');
    const string = Object.keys(objetoRetornado).toString();
    assert.deepStrictEqual(string.includes('fetchMenu'), true);
    ...
    objetoRetornado = createMenu({
      food: { pizza: 15.0 },
      drink: { refrigerante: 7.5 },
    });
    const objectKeys = Object.keys(objetoRetornado.fetchMenu()); // Retorno: { food: {}, drink: {}}
    assert.deepStrictEqual(objectKeys, ['food', 'drink']);
    ...
    const objectFetched = objetoRetornado.fetchMenu(); // Retorno: { food: {}, drink: {}}
    assert.deepStrictEqual(objectFetched, {
      food: { pizza: 15.0 },
      drink: { refrigerante: 7.5 },
    });
    ...
    objetoRetornado.order('coxinha');
    // // objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos
    assert.strictEqual(objetoRetornado.pay(), parseFloat((14 * 1.1).toPrecision(5)));
