...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const keysObject = ['food', 'drink'];
    assert.deepStrictEqual(Object.keys(objetoRetornado.fetchMenu()), keysObject);
    ...
    let segundoObjetoRetornado = createMenu(objetoQualquer);
    segundoObjetoRetornado.order('coxinha');
    segundoObjetoRetornado.order('agua');
    segundoObjetoRetornado.order('coxinha');
    // objetoRetornado.comsuption // Retorno: ['coxinha', 'agua', 'coxinha']
    assert.deepStrictEqual(segundoObjetoRetornado.consumption, ['coxinha', 'agua', 'coxinha']);
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    // ```
    const total = segundoObjetoRetornado.pay(); // Retorno: somaDosPreçosDosPedidos
    assert.strictEqual(total, 11.7 * 1.1);
