...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    // ```
    const fetchMenuLenght = Object.keys(objetoRetornado.fetchMenu()).length;
    const fetchMenuFood = objetoRetornado.fetchMenu().food !== undefined;
    const fetchMenuDrink = objetoRetornado.fetchMenu().drink !== undefined;

    assert.deepStrictEqual([fetchMenuLenght, fetchMenuDrink, fetchMenuFood], [2, true, true]);
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    // const objetoRetornado = createMenu(objetoQualquer);
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer
    // ```
    assert.deepStrictEqual(objetoRetornado.fetchMenu(), { drink: {}, food: {} });
    ...
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    assert.deepStrictEqual(objetoRetornado.pay(), (3.9 + 3.9 + 3.9) * 1.10);
