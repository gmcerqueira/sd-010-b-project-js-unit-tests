...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    assert.deepStrictEqual(objRetornado.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);

    // ```
    // Agora faça o TESTE 7 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 7: Verifique que a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    const objRetorn = createMenu({});
    // objetoRetornado.order('coxinha');
    objRetorn.order('coxinha');
    // objetoRetornado.order('agua');
    objRetorn.order('agua');
    // objetoRetornado.order('coxinha');
    objRetorn.order('coxinha');
    // objetoRetornado.comsuption // Retorno: ['coxinha', 'agua', 'coxinha']
    assert.deepStrictEqual(objRetorn.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    objRetornPay.order('tereré');
    // objetoRetornado.order('cerveja');
    objRetornPay.order('cerveja');
    // objetoRetornado.order('arroz_pantaneiro');
    objRetornPay.order('arroz_pantaneiro');
